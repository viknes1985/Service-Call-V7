...
async function startServer() {
  ...
  app.get("/api/services", async (req, res) => {
    const { state, town, category, search, createdBy, status, isAdmin } = req.query;
    let filter: any = {};
        ...
        try {
          await resend.emails.send({
            ...
            html: `
              ...
              ${status === 'approved' ? '<p>It is now visible to all users.</p>' : '<p>Please check your profile for details.</p>'}
            `
          });
        } catch (emailErr) {
  ...
  app.post("/api/services/:id/nudge", async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        console.error("Admin nudge email failed:", emailErr);
      }
